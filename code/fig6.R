...
deg <- read.table("THCA/TCGA_THCA_DESeq2_symbol_results.txt",header = T,sep = "\t",quote = "",check.names = F)

lr_cor <- readRDS("LR/lr_cor.RDS")
lr_cor <- lapply(lr_cor, function(x){
  filter(x,Padj < 0.05 & cor > 0)
})
for (i in names(lr_cor)) {
...
deg_sub <- deg[deg$Gene_symbol %in% c(markerL$gene,markerR$gene),]
deg_sub <- deg_sub[order(deg_sub$FDR),]

Exp <- read.table("THCA/TCGA_THCA_Gene_fpkm_profile_symbol.txt",
                  header = T,quote = "",sep = "\t",check.names = F,row.names = 1)
normal <- names(Exp)[substr(names(Exp),14,14) == "1"]
samplePair <- substr(normal,1,12)
sampleID <- names(Exp)[substr(names(Exp),1,12) %in% samplePair]
sampleID <- sampleID[order(substr(sampleID,14,14),sampleID)]

Exp_sub <- Exp[,sampleID]

Exp_L <- Exp_sub[rownames(Exp_sub) %in% markerL$gene,]
Exp_L <- Exp_L[rowSums(Exp_L > 0) > 0,]
Exp_L <- t(scale(t(Exp_L)))
Exp_R <- Exp_sub[rownames(Exp_sub) %in% markerR$gene,]
Exp_R <- Exp_R[rowSums(Exp_R > 0) > 0,]
Exp_R <- t(scale(t(Exp_R)))

group <- substr(sampleID,14,14)
group <- group %>% gsub("0","Tumor",.) %>% gsub("1","Normal",.)

Ligand <- rownames(Exp_L)
Ligand <- ifelse(Ligand %in% deg$Gene_symbol[deg$log2FoldChange > log2(1.5) & deg$FDR < 0.05],"up",Ligand) %>% 
  ifelse(. %in% deg$Gene_symbol[deg$log2FoldChange < -log2(1.5) & deg$FDR < 0.05],"down",.) %>% 
  ifelse(. %in% c("up","down"),.,"stable")
Receptor <- rownames(Exp_R)
Receptor <- ifelse(Receptor %in% deg$Gene_symbol[deg$log2FoldChange > log2(1.5) & deg$FDR < 0.05],"up",Receptor) %>% 
  ifelse(. %in% deg$Gene_symbol[deg$log2FoldChange < -log2(1.5) & deg$FDR < 0.05],"down",.) %>% 
  ifelse(. %in% c("up","down"),.,"stable")

colanno <- ComplexHeatmap::HeatmapAnnotation(
  tissue = group,
  col = list(tissue = c("Normal" = "blue",
                        "Tumor" = "red"))
)
rowanno_L <- ComplexHeatmap::rowAnnotation(
  Ligand = Ligand,
  col = list(Ligand = c("up" = "#FC4E07",
                        "stable" = "grey",
                        "down" = "#00AFBB"))
)
rowanno_R <- ComplexHeatmap::rowAnnotation(
  Receptor = Receptor,
  col = list(Receptor = c("up" = "#FC4E07",
                          "stable" = "grey",
                          "down" = "#00AFBB"))
)

pdf("fig6kl.pdf",height = 8,width = 10)
ComplexHeatmap::Heatmap(Exp_L,name = "z.score",
                        show_column_names = F,
                        cluster_columns = F,
                        show_row_dend = F,
                        top_annotation = colanno,
                        left_annotation = rowanno_L)
ComplexHeatmap::Heatmap(Exp_R,name = "z.score",
                        show_column_names = F,
                        cluster_columns = F,
                        show_row_dend = F,
                        top_annotation = colanno,
                        left_annotation = rowanno_R)
dev.off()
